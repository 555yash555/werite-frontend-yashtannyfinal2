...
const PostsCard = ({ townHall,   live }) => {
  ...
  const handleCopyLink = () => {
    ...
    navigator.clipboard.writeText(postLink)
      .then(() => {
        ...
        <Message variant='success'>Your post has been created successfully.</Message>
      })
      .catch((error) => {
  ...
  const handleShowComments = () => {
    console.log("reaction triggerred");
...
>
  <BsBookmark size={40} style={{ color: bookMark ? "red" : "black" }} onClick={() => handleBookmark(townHall.post_id)}/>
    ...
    >
    <FaShareAlt  size={42} onClick={handleCopyLink} style={{ position: 'relative', bottom: '-0.4rem' }}/>
  </Link>
    </div>
  </Col>
</Row>
  ...
  <Col xs={5} >
    ...
    <div>
      ...
      style={{ font: "inherit", textDecoration: "inherit",color:"black" }}
    >
    <FaShareAlt size={40} onClick={handleCopyLink} style={{ position: 'relative', bottom: '-0.4rem' }}/>
  </Link>
    </div>
  </Col>
</Row>
