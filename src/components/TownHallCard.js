...
const TownHallCard = ({ townHall, showId, setShowId, live }) => {
  ...
  const [totalLikeAndDislikeCount, settotalLikeAndDislikeCount] = useState(upVoteCount+downVoteCount);
  ...
  const handleCopyLink = (event) => {
    ...
    const postLink = `${window.location.origin}/app/singlepost/${townHall.post_id}?source=werite&medium=Sharebutton`;
  ...
  const handleAddLike = (post_id) => {
    ...
    if(!thumbsDown)settotalLikeAndDislikeCount(totalLikeAndDislikeCount+1);
    setThumbsDown(false);
    setThumbsUp(true);
    dispatch(addLike(post_id));
    console.log(thumbsUp);
    setUpVoteCount(upVoteCount + 1);
    if(thumbsDown)setDownVoteCount(downVoteCount - 1);
  };
  const handleremovedisLike = (post_id) => {
    dispatch(removeDislike(post_id));
    setThumbsDown(false);
    settotalLikeAndDislikeCount(totalLikeAndDislikeCount-1);
  ...
  const handleAddDisLike = (post_id) => {
    ...
    if(!thumbsUp)settotalLikeAndDislikeCount(totalLikeAndDislikeCount+1);
    setThumbsUp(false);
    setThumbsDown(true);
  ...
  const Block = useSelector((state) => state.blockReducer);
  let { berror,bsuccess } = Block;
  ...
  let { loading,error,success } = Report;
  ...
  alignItems: "flex-start" }}>
    ...
    />
        ...
        <Modal.Footer>
          ...
          {/* {success && <Message variant='success'>Your Report has been created successfully.</Message>}
  ...
  useEffect(() => {
    dispatch({ type: 'BLOCK_CREATE_RESET' });
      ...
      >
        ...
        >
          ...
          <div style={{ height: "inherit", margin: "15px 0" }}>
            ...
            {bsuccess && <Message variant='success'>User Blocked</Message>}
    {berror && <Message variant='danger'>{error}</Message>}
...
<Link
    ...
    >
    <FaShareAlt size={40} onClick={(event) => handleCopyLink(event)} style={{ position: 'relative', bottom: '-0.4rem' }}/>
      ...
      >
          ...
          >
            ...
            <div style={{ height: "inherit", margin: "15px 0" }}>
              ...
              {bsuccess && <Message variant='success'>User Blocked</Message>}
    {berror && <Message variant='danger'>{berror}</Message>}
    {success && <Message variant='success'>Your Report has been created successfully.</Message>}
